...
3. [สถานะปัจจุบัน (snapshot)](#สถานะปัจจุบัน-snapshot)
4. [โครงสร้าง repo](#โครงสร้าง-repo)
5. [เริ่มพัฒนาในเครื่อง](#เริ่มพัฒนาในเครื่อง)
6. [รัน Worker ในเครื่อง](#รัน-worker-ในเครื่อง)
7. [Endpoints](#endpoints)
8. [Cron / คิวประมวลผล](#cron--คิวประมวลผล)
9. [State machine ของงาน](#state-machine-ของงาน)
10. [Visibility model (ใครเห็นงานใคร)](#visibility-model-ใครเห็นงานใคร)
11. [แชทบอทน้องกุ้ง (query engine)](#แชทบอทน้องกุ้ง-query-engine)
12. [ฐานข้อมูล (D1)](#ฐานข้อมูล-d1)
13. [สะพานเชื่อม LINE (`bridge/`)](#สะพานเชื่อม-line-bridge)
14. [Deploy จริง](#deploy-จริง)
15. [Dump ฐานข้อมูล](#dump-ฐานข้อมูล)
16. [ความปลอดภัย / ข้อมูลส่วนบุคคล](#ความปลอดภัย--ข้อมูลส่วนบุคคล)
17. [CI](#ci)
18. [เทส](#เทส)
19. [กติกาที่ต้องรู้ก่อนแตะของจริง](#กติกาที่ต้องรู้ก่อนแตะของจริง)
20. [เอกสารอื่น ๆ ในรีโปนี้](#เอกสารอื่น-ๆ-ในรีโปนี้)
...
ตัวเลขด้านล่างเป็น**ภาพนิ่ง ณ วันที่เขียน (2026-08-28)** — ห้ามอ้างต่อว่าเป็นสถานะล่าสุด
อ่านของจริงที่ [`CLAUDE.md` §3](./CLAUDE.md) เสมอก่อนรายงานสถานะให้ใครฟัง:

- **P0** (ท่อ + กระดูก): เดินจริงแล้ว แต่ยังไม่มีใบปิดทางการ
- **P1** (ประสาทสัมผัส — ปุ่มบน LINE เปลี่ยนสถานะงานจริง): **ปิดจบ 100%** พิสูจน์วงจรเต็มด้วยข้อมูลจริงแล้ว
- **P2 ชุด A** (สายตา — visibility model + query engine): เขียนเสร็จ ผ่านตรวจอิสระ 3 รอบ แต่ยังไม่ปิดทางการ
- **P2 ชุด B** (ปาก — endpoint `/ask` + สรุปเช้า-เย็น แตะ LIVE): ยังไม่เริ่ม รออนุมัติแยก
- **P3–P5**: ยังไม่เขียนใบสั่งงาน

ของจริงที่รันอยู่ (ตามบันทึกล่าสุด): Worker `aim-ingest` (`https://aim-ingest.mottosolar.workers.dev`) ·
D1 `aim-db` · Cron ทุก 1 นาที · endpoint `POST /ingest/line` + `GET /healthz` · migration 3 ใบ
...
Worker มีแค่ 2 เส้นทางเท่านั้น (จับคู่ตรงด้วย `pathname`, ตอบ JSON เสมอ):

| Method | Path | Auth | คำอธิบาย |
|---|---|---|---|
| `GET` | `/healthz` | ไม่ต้อง | คืน `{ ok, ts, queueDepth, lastIngestAt }` (200) หรือ `{ ok:false, ... }` (503 ถ้า D1 ต่อไม่ติด) |
| `POST` | `/ingest/line` | header `X-AIM-Key` ต้องตรงกับ secret `AIM_INGEST_KEY` (เทียบแบบ constant-time) | รับ LINE webhook envelope (`{ events: [...] }`, สูงสุด 50 event/ครั้ง), แฮช source id ด้วย `AIM_SOURCE_HASH_SALT`, แล้ว enqueue แบบ idempotent |

`POST /ingest/line` ตรวจ payload อย่างเข้มก่อนรับ (โครง event, `webhookEventId` ต้องไม่ใช่ ULID ดิบของ LINE,
ชนิด event/message ที่รู้จัก, `source.type` ที่ถูกต้อง ฯลฯ) — ผิดรูปแบบตอบ `400`, key ผิดตอบ `403`,
ยังไม่ตั้งค่า secret หรือ D1 ล่มตอบ `503` เส้นทางอื่นนอกจากสองเส้นนี้ตอบ `404`

**ยังไม่มี HTTP endpoint สำหรับ task CRUD หรือถามคำถามผ่านแชท** — query engine (น้องกุ้ง) มีโค้ด/เทสพร้อมแล้ว
แต่ยังไม่ถูกเปิดผ่าน endpoint ใด ๆ (รอ WP-P2-B2 ซึ่งยังไม่เริ่ม)
...
3. ผูกผู้กดปุ่ม (source hash) เข้ากับ `person_code` ผ่านตาราง `person_link` แล้วเรียก state machine
4. ถ้าล้มเหลว: retry แบบ exponential backoff (`30 × 2^(attempts-1)` วินาที) สูงสุด 5 ครั้ง ก่อนย้ายไปสถานะ `dead`
5. บันทึกทุกรอบลง `ledger` (ไม่ว่าจะมีงานให้ทำหรือไม่)
