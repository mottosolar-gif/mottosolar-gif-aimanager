...
③ cim-server  `C:\WebApp\vendor-ksk` (PHP 8.1.9 · MySQL db_customs)              [อ้างเอกสาร — ไม่เห็นของจริง]
   ...
   ├─ ปุ่มการ์ด → ll_task_postback() เขียน tbl_task (ความจริงของงาน · D-P0-17)
...
| `outbound_queue` | `idem_key` UNIQUE · FK→task | (ว่างมาตลอด — D-P0-17) |
...
| `lib/holiday.php` → `tbl_holiday` | ด่านปฏิทิน (ตอบ 412 ให้คลาวด์จด `skipped_holiday`) | D-P0-15 §2 |
...
| 2 | Impact ก่อนแก้ table/field/module | เหมือน 1 + `imports/calls` (Graphify) + smoke test ที่ครอบไฟล์นั้น | ⚠ ได้ครึ่ง (โค้ด→โค้ด) | ลิสต์ "ไฟล์ · task · report · เทส" ที่แตะคอลัมน์ ก่อนเขียน migration — ปิดบั๊กแบบ D-P0-17 (`applyTransition` เขียนสำเนาแล้วถูก sync ทับ) ที่เอกสารรู้แต่โค้ดไม่รู้ | กราฟเก่ากว่าโค้ด ⇒ ทุกคำตอบต้องบอก `graph_built_at` + `git rev` · ถ้าไฟล์เปลี่ยนหลังสร้างกราฟต้องติดธง `stale` |
...
| 6 | วิเคราะห์ incident · ค้นเหตุผลการตัดสินใจเดิม | `DECISIONS_P0.md` (D-P0-01..18) · คอมเมนต์ `★ YYYY-MM-DD` ในโค้ด · `# WHY:`/`# NOTE:` | ⚠ Graphify ทำ `# NOTE:/# WHY:` เป็น node ได้ (README) · ไม่รู้จักรูปแบบ `★ 2026-08-28 (D-P0-17)` และคอมเมนต์ไทย · ต้องใช้ LLM กับ `.md` | เชื่อม "โค้ดบรรทัดนี้" ↔ "คำตัดสินใบนี้" — กัน "ถามซ้ำเรื่องที่ปิดแล้ว" ที่ D-P0-16/18 บันทึกว่าเกิดซ้ำ | เอกสารมีชื่อเล่น/ตัวเลขจริง ⇒ semantic pass ต้องรันในบ้านเท่านั้น หรือใช้ regex extractor เฉพาะ (`D-P0-\d+` · `★ 20\d\d-\d\d-\d\d`) ซึ่งไม่ต้องใช้ LLM เลย |
...
1. **ดัชนี ไม่ใช่ความจริง** — ทุก node/edge มี `evidence[]` ที่ชี้กลับได้ (`file:line` · `table.column` · `schtasks:<name>` · `doc:D-P0-17`) · node ที่ไม่มี evidence ห้ามมี
...
| `Decision` | `D-P0-17` | date · title · supersedes | `D-P0-17 tbl_task คือความจริง` |
...
| `mirrors` | Table→Table (สำเนาทางเดียว) | MANUAL/DERIVED — `aim-db.task mirrors db_customs.tbl_task` (D-P0-17) |
...
| `fim_impact` | `target` (column/table/module/function) · `depth` (≤3) | จัดกลุ่ม: `code[]` · `jobs[]` · `reports[]` · `notifications[]` · `tests[]` · `decisions[]` · `mirrors[]` + `confidence_summary` | "ถ้าเปลี่ยน ENUM `tbl_task.Status` กระทบอะไร" → ต้องมี `mirrors: aim-db.task` และ `decisions: D-P0-17` และเตือน P4 "ห้ามเปลี่ยนสิ่งที่ถูกเทียบ" (`WORK_PACKAGES_P4.md:22`) |
...
| 3 | เปลี่ยน ENUM `tbl_task.Status` กระทบอะไร (Pilot-B) | `fim_impact(db_customs.tbl_task.Status)` | `mirrors → aim-db.task` · `decisions: D-P0-17` · `tasks.php`/`work_items.php`/`task_remind` (`DATABASE_SCHEMA.md:76`) · เตือน P4 | มี `mirrors`+`decisions` · ไม่มี false positive เกิน 1 |
...
| 7 | report ไหนไม่มี producer หรือ consumer | `fim_graph_query("Report orphan")` | Pilot-A: `outbound_queue` = ช่องส่งที่ไม่มีใครเขียน (in-degree writes=0 — D-P0-17) · Pilot-B: ตามจริง | ผลตรงกับการ grep มือ 100% |
...
| 9 | ทำไม `POST /admin/summary` ถึงตอบ 412 เป็น skipped_holiday | `fim_get_node` + `fim_graph_query` | `D-P0-15 §2` · `repository.ts:15-24` · `outbound.ts` แยก 4xx | คืน Decision node + บรรทัดโค้ดที่อ้าง |
...
| 3 | กราฟกลายเป็น "ความจริงชุดที่สอง" — คนแก้กราฟแทนแก้โค้ด หรือ agent เชื่อกราฟที่เก่ากว่าโค้ด | 🔴 | กราฟ regenerate ทั้งก้อนทุกครั้ง (ไม่ incremental — เหตุผลเดียวกับ D-P0-17 "ส่งทั้งชุดทุกรอบ") · `status: stale` เมื่อ `git_rev ≠ HEAD` · ห้าม `--strict` |
...
| 10 | ทำซ้ำเรื่องที่ตัดสินแล้ว / ข้ามประตู (บทเรียน D-P0-16/18) | 🟡 | งานนี้ **ไม่อยู่ในเฟส P0–P5** ของ AI Manager — เป็นเครื่องมือของผู้ตรวจ ไม่กระทบ P4 (ไม่แตะสิ่งที่ถูกเทียบ) · ต้องเขียนไว้ชัดในใบตัดสิน D-KG-01 ว่าไม่ใช่การเริ่ม P5 |
